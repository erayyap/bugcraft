...
def remove_overlap(boxes, iou_threshold, ocr_bbox=None):
    ...
    def box_area(box):
        return max((box[2] - box[0]) * (box[3] - box[1]), 1e-10)
...
def annotate(image_source: np.ndarray, boxes: torch.Tensor, logits: torch.Tensor, phrases: List[str], text_scale: float, 
    ...
    from action_model.OmniParser.util.box_annotator import BoxAnnotator 
    box_annotator = BoxAnnotator(text_scale=text_scale, text_padding=text_padding,text_thickness=text_thickness,thickness=thickness, )#avoid_overlap=False
